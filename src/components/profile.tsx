...
function Profile() {
  ...
  useEffect(() => {
    ...
    const july31: Date = new Date(today.getFullYear(), 6, 31); 

    const isAfterJuly31: boolean = today > july31;
    console.log(isAfterJuly31);
    if (isAfterJuly31) {
      setIsAllowed(true);
    }
  }, []);
  ...
  return (
    <>
      ...
      {showModal ? (
        <AlertModal
          title={"Not Available"}
          description="This Resume is not available right now "
          handleModal={handleDownloadIssue}
        />
      ) : null}
    </>
  );
